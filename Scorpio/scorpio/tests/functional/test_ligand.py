...
class TestLigandController(object):

    def __init__(self, request):
        self.request = request

    def test_view(self):
        response = self.app.get(url(route_name='ligand_v', id=1))
        # Test response...
        assert 'Ligand' in response 

    def test_view_404_no_id(self):
        response = self.app.get(url(route_name='ligand_v'),
                                status=404)
    ...
    def test_browse(self):
        response = self.app.get(url(route_name='ligand_br'))
        # Test response...
    ...
    def test_itc_data(self):
        response = self.app.get(url(route_name='ligand_itc', id=1))
        # Test response...
        assert 'Browse ITC data' in response

    def test_itc_data_404_no_id(self):
        response = self.app.get(url(route_name='ligand_itc'),
                                status=404)
    ...
    def test_structure_data(self):
        response = self.app.get(url(route_name='ligand_structure', id=1))
        # Test response...
        assert 'Browse structures' in response

    def test_structure_data_404_no_id(self):
        response = self.app.get(url(route_name='ligand_structure'),
                                status=404)
